...
| API-PLAN-010 | GET | `/api/plans` | 내 여행 저장 일정 목록 | **2팀과 합의** |
| API-PLAN-011 | DELETE | `/api/plans/{planId}` | 저장 일정 삭제 | **2팀과 합의** |
...
### API-PLAN-010 / 011 · 내 여행 목록 · 삭제 (2팀과 합의 필요)

같은 테이블을 쓰므로 모양만 제안합니다. `save_yn = TRUE`인 일정만 대상.
...
| 2 | 내 여행 목록·삭제(PLAN-010·011) 담당 | 2팀(내 여행 화면 담당)이 구현, `PlanMapper`는 공유 |
