...
- 좌표가 있음
- 영업 시작·종료 시간이 있음
- 원문에서 명확히 파싱된 브레이크타임에는 속하지 않음

TourAPI는 폐업 여부를 확정하는 상태값을 제공하지 않으므로, 폐업을 추정해 삭제하지 않는다. `준비시간`, `브레이크타임`, `휴게시간`, `휴식시간`으로 명확히 적힌 한 구간은 구조화한다. 요일별·복수 구간 등 애매한 원문은 그대로 보존하고 자동 추천에서 별도 확인한다.
